...
function broadcastRoomState(io, code, room) {
  ...
  io.to(roomChannel(code)).emit('snakeGameLobby:state', payload)
  for (const slot of Array.isArray(room?.slots) ? room.slots : []) {
    if (slot?.userId) emitToUser(io, String(slot.userId).trim(), 'snakeGameLobby:state', payload)
  }
}

/** Bắt đầu trận — room + user:id. */
function broadcastSnakeGameStarted(io, code, room, startedPayload) {
  io.to(roomChannel(code)).emit('snakeGameLobby:started', startedPayload)
  for (const slot of Array.isArray(room?.slots) ? room.slots : []) {
    if (slot?.userId) emitToUser(io, String(slot.userId).trim(), 'snakeGameLobby:started', startedPayload)
  }
}
  ...
  try {
    ...
    if (groupMembers.length === capacity) {
      ...
      groupMembers.forEach(m => {
        ...
        io.to(`user:${m.userId}`).emit('snakeGameLobby:started', sanitizeObject({
          roomCode: code,
          aiResult,
          isAutoMatched: true,
      ...
      io.emit('snakeGameLobby:matchFoundGlobal', sanitizeObject({
        roomCode: code,
        userIds: groupIds,
        fullRoom: room
      }))
...
export function registerSnakeGameLobbyHandlers(io, socket) {
  if (socket.userId) {
    ...
    if (cur) {
      ...
      if (room) {
        ...
        if (room.slots.filter(Boolean).length === 0) {
          ...
          io.to(roomChannel(cur)).emit('snakeGameLobby:state', sanitizeObject(room))
        }
      }
  ...
  socket.on('snakeGameLobby:leave', () => {
    leaveRoom()
  })

  socket.on('snakeGameLobby:create', async (payload, ack) => {
    try {
      ...
      socket.emit('snakeGameLobby:state', sanitizeObject(room))
    } catch (e) {
      ack?.({ ok: false, error: e?.message || 'create_failed' })
    }
  })

  socket.on('snakeGameLobby:join', async (payload, ack) => {
    try {
      ...
      if (getLobbyRoomCode(socket) === code) {
        ...
        if (myIdx >= 0) {
          ...
          io.to(roomChannel(code)).emit('snakeGameLobby:state', sanitizeObject(room))
          return
        }
      }
  ...
  socket.on('snakeGameLobby:setReady', (payload) => {
    const cur = getLobbyRoomCode(socket)
    if (!cur) return
    const room = rooms.get(cur)
  ...
  socket.on('snakeGameLobby:chat', (payload) => {
    const cur = getLobbyRoomCode(socket)
    if (!cur) return
    const room = rooms.get(cur)
    ...
    if (profile && payload?.text) {
      ...
      io.to(roomChannel(cur)).emit('snakeGameLobby:chatMessage', msg)
    }
  })

  socket.on('snakeGameLobby:start', async (_payload, ack) => {
    ...
    if (occupied.length >= room.capacity) {
      ...
      broadcastSnakeGameStarted(io, code, room, startedPayload)
      return ack?.({ ok: true })
    }
    ...
    broadcastLobbyEventToRoom(io, code, room, 'snakeGameLobby:matching', {})
  ...
  socket.on('snakeGameLobby:cancelStart', (_payload, ack) => {
    const code = getLobbyRoomCode(socket)
    ...
    if (room.lastMatchRequest) delete room.lastMatchRequest
    room.findingMatch = false
    broadcastLobbyEventToRoom(io, code, room, 'snakeGameLobby:matchingEnd', {})
    broadcastRoomState(io, code, room)
    ack?.({ ok: true })
  })

  socket.on('snakeGameLobby:findMatch', async (payload, ack) => {
    try {
      const capacity = Number(payload?.capacity)
      if (!ALLOWED_CAPACITIES.includes(capacity)) return ack?.({ ok: false, error: 'invalid_capacity' })
      leaveRoom()
  ...
  socket.on('snakeGameLobby:invite', async (payload, ack) => {
    ...
    emitToUser(io, friendId, 'snakeGameLobby:inviteReceived', {
      inviterName,
